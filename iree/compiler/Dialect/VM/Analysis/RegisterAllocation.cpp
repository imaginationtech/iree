...
struct RegisterUsage {
  llvm::BitVector intRegisters{Register::kIntRegisterCount};
  ...
  Optional<int> findFirstUnsetIntOrdinalSpan(size_t byteWidth) {
    ...
    while (ordinalStart != -1) {
      ...
      if (ordinalEnd >= Register::kIntRegisterCount) {
        return llvm::None;
      }
...
LogicalResult RegisterAllocation::recalculate(IREE::VM::FuncOp funcOp) {
  ...
  if (maxI32RegisterOrdinal_ > Register::kIntRegisterCount ||
...
RegisterAllocation::remapSuccessorRegisters(Operation *op, int successorIndex) {
  ...
  if (scratchI32Reg != maxI32RegisterOrdinal_) {
    ...
    assert(getMaxI32RegisterOrdinal() <= Register::kIntRegisterCount &&
           "spilling i32 regs");
    if (getMaxI32RegisterOrdinal() > Register::kIntRegisterCount) {
